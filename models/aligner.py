...
class LengthClassDuplicationModule(nn.Module):
    def __init__(self, input_dim, hidden_dim, max_duplication=3):
        ...
        self.relu = nn.ReLU()
        self.projection = nn.Linear(hidden_dim, max_duplication * 3)
    ...
    def forward(self, x):
        ...
        duplication_matrix = torch.zeros((x.shape[0], x.shape[1] * self.max_duplication, x.shape[1]), dtype=torch.float32) + torch.arange(1, 1 + x.shape[1] * self.max_duplication, dtype=torch.float32).unsqueeze(0).unsqueeze(2)
...
class GroupingModule(nn.Module):
    ...
    def forward(self, x):
        ...
        groups = torch.cumsum(torch.ones(groups.shape, dtype=torch.float32).to(device=x.device), dim=-1)
        groups[:, 0] = 0
        groups[start_groups[0], start_groups[1]] = 0
        groups[:, -1] = last_group
        groups = torch.gather(groups, 1, groups.ne(0).to(dtype=torch.int32).argsort(dim=1, descending=True, stable=True)).unsqueeze(2)
        grouping_matrix = torch.zeros((x.shape[0], x.shape[1], x.shape[1]), dtype=torch.float32) + torch.arange(1, 1 + x.shape[1], dtype=torch.float32).unsqueeze(0).unsqueeze(1)
...
class PermutationModule(nn.Module):
    def __init__(self, input_dim, hidden_dim, attention_dim):
        ...
        self.attention = Attention(hidden_dim, attention_dim, heads=1)
        self.gated_attention = GatedAttention(hidden_dim, attention_dim)

    def forward(self, x):
        for layer in self.encoder:
            x = layer(x)
        context_vector = self.attention(self.pre_net(x))
        return self.gated_attention(context_vector)
